...
class PdfTool(QMainWindow):
	def __init__(self):
		...
		self.btn2.clicked.connect(lambda : self.pdf_splitter(self.getSelectedItem(),plst))
	...
	def pdf_splitter(self,path,plst):
		plst = self.getPages()
		try:
			...
			if plst[0]<plst[1] and plst[1]<totalP:
				...
				output_filename = '{}_page_{}.pdf'.format(fname, str(plst[0])+'_'+str(plst[1]))
				...
				if plst[0]>0:
					...
					output_filename = '{}_page.pdf'.format(fname)
					with open(output_filename, 'wb') as out:
						pdf_writer.write(out)
				else:
					pdf_writer = PdfFileWriter()
